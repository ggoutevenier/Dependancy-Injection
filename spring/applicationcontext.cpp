...
#include <dlfcn.h>
	...
	IBean *ApplicationContext::getBean(std::string name) {
		Beans::const_iterator it = beans.find(name);
		if (it == beans.end())
			throw std::runtime_error("bean not found " + name);
		IBean *bean = it->second.get();
		loadDep(bean);
		bean->construct();
		return bean;
	}
	void ApplicationContext::loadDep(IBean *bean) {
	...
	void ApplicationContext::readXML(const std::string &fileName) {
		...
		BOOST_FOREACH(boost::property_tree::ptree::value_type &bean, pt.get_child("beans")) {
			...
			BOOST_FOREACH(boost::property_tree::ptree::value_type &prop, bean.second.get_child("properties")) {
